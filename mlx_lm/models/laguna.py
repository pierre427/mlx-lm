# Laguna (poolside) MoE support for mlx-lm.
# Ported from ml-explore/mlx-lm PR #1223 (Blaizzy:pc/add-lg), which adds
# LagunaForCausalLM. Local addition: sanitize() strips the `language_model.`
# wrapper prefix used by the AtomicChat 8-bit repack of poolside/Laguna-XS-2.1.
...
class LagunaModel(nn.Module):
    ...
    def __call__(
        ...
        # Optional EAGLE-3 aux hidden-state capture for the DFlash speculator:
        # collect the post-layer hidden state at the requested layer indices.
...
class Model(nn.Module):
    ...
    def make_cache(self):
        # 30 of 40 layers are sliding_attention (window 512): they never attend
        # beyond the window, so a bounded RotatingKVCache is both correct and far
        # cheaper than a full cache at long context. Only the 10 full_attention
        # (global, YARN) layers need an unbounded KVCache.
        caches = []
        for lt in self.args.layer_types:
            # A sliding layer needs a valid window; if a (malformed) config omits
            # sliding_window, fall back to a full cache rather than crash later on a
            # None max_size comparison.
    ...
    def sanitize(self, weights):
        ...
        # tensor under a VLM-style `language_model.` prefix. Strip it so keys
        # line up with this module tree (model.* / lm_head.*).
